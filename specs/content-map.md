...
work. The repo ships three skills — `sdd-init`, `sdd-plan`, and `sdd-make` —
each authored as a `SKILL.md` instruction file and installed into a user's
...
| Path | Purpose |
|------|---------|
| `README.md` | User-facing overview: the three skills, installation, and layout. |
...
| `skills/sdd-init/SKILL.md` | Bootstrap SDD scaffolding in a repo (this skill). |
...
Each skill is a directory under `skills/` containing a single `SKILL.md` with
YAML frontmatter (`name`, `description`) followed by the instructions Claude
follows when the skill is invoked. There is no executable code.

- **`sdd-init`** — `skills/sdd-init/SKILL.md`: creates `specs/` (content map +
  ADRs) and ensures `AGENTS.md` exists with a `CLAUDE.md` symlink.
...
The skills form a workflow: `sdd-init` → `sdd-plan` → `sdd-make`.
